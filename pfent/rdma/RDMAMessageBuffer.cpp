...
vector<uint8_t> RDMAMessageBuffer::receive() {
    ...
    // | size_of_receive | data_of_receive | validity(0xDEADDEADBEEFBEEF) |
    // 从当前位置反复读收下一个数据的validity，判断数据是否已经完成发送
    do {
...
void RDMAMessageBuffer::send(const uint8_t *data, size_t length, bool inln) {
    ...
    // | size_of_send | data_of_send | validity(0xDEADDEADBEEFBEEF) |
