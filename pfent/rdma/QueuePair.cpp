          ...
          , completionQueuePair(completionQueuePair)
{
   ...
   queuePairAttributes.cap.max_send_wr = 2048;                    // Requested max number of outstanding WRs in the SQ
   queuePairAttributes.cap.max_recv_wr = 2048;                    // Requested max number of outstanding WRs in the RQ
...
void QueuePair::connect(const Address &address, unsigned retryCount)
{
   ...
   attributes.max_rd_atomic = 16;     // The number of outstanding RDMA reads & atomic operations (initiator)
